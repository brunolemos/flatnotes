...
namespace JWT
{
    ...
    public static class JsonWebToken
    {
        ...
        static JsonWebToken()
        {
            ...
            {
                { JwtHashAlgorithm.HS256, (key, value) => { return HMACSHA256( key, value ); } },
                //{ JwtHashAlgorithm.HS384, (key, value) => { using (var sha = new HMACSHA384(key)) { return sha.ComputeHash(value); } } },
                //{ JwtHashAlgorithm.HS512, (key, value) => { using (var sha = new HMACSHA512(key)) { return sha.ComputeHash(value); } } }
            };
        }

        public static byte[] HMACSHA256(byte[] secretKey, byte[] value)
        {
            // Move strings to buffers.
            var key = CryptographicBuffer.ConvertStringToBinary( Convert.ToBase64String(secretKey), BinaryStringEncoding.Utf8);
            var msg = CryptographicBuffer.ConvertStringToBinary( Convert.ToBase64String(value), BinaryStringEncoding.Utf8);
            ...
            string str = CryptographicBuffer.EncodeToHexString(hash.GetValueAndReset());
        ...
        public static string Decode( string token, byte[] key, bool verify = true )
        {
            ...
            var headerJson = Convert.ToBase64String( Base64UrlDecode( header ) );
            var headerData = JsonConvert.DeserializeObject<Dictionary<string, object>>( headerJson );
            var payloadJson = Convert.ToBase64String( Base64UrlDecode( payload ) );
